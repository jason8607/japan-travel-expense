import type { Category, OCRResult } from "@/types";
...
function normalizeCategory(value: unknown): Category {
  if (typeof value === "string" && (OCR_CATEGORIES as readonly string[]).includes(value)) {
    return value;
  }
  return "其他";
}
...
5. **分類擷取** — 為每個品項從以下 9 個固定分類擇一（必須完全相符，不可自創、縮寫或翻譯）：
   - 餐飲：食品、飲料、甜點、便當、麵類、咖啡、酒類、零食、調味料、米麵
   - 交通：JR / 私鐵 / 地下鐵 / 巴士 / 計程車 / 機場接駁 / 加油 / 停車費 / IC 卡儲值
   - 購物：伴手禮、紀念品、雜貨、玩具、3C 配件、文具、生活雜物、廚房用具
   - 住宿：旅館 / 民宿 / 飯店住房費 / 客房加值服務
   - 門票：景點門票、博物館、主題樂園、演唱會、特展、觀光巴士套票
   - 藥品：處方藥、OTC 藥、保健食品、繃帶、口罩、體溫計、護理用品
   - 美妝：化妝水、乳液、唇膏、粉底、防曬乳、卸妝、面膜、美容液、洗面乳、香水
   - 衣服：T 恤、褲子、裙子、外套、洋裝、襪子、內衣、帽子、圍巾、手套、鞋子、配件
   - 其他：看不懂或無法歸類時用「其他」，不要硬塞「餐飲」

   即使整張收據看起來像同一類（例如便利商店裡也可能混食物與日用品），仍要逐品項判斷。
  ...
  "items": [
    {
      ...
      "category": "餐飲|交通|購物|住宿|門票|藥品|美妝|衣服|其他"
    }
  ],
  "total": 總金額日幣,
...
export async function recognizeReceipt(
  ...
  const validatedItems = parsed.items.map((item, i) => {
    ...
    return {
      ...
      tax_rate: taxRate,
      tax_type: taxType,
      category: normalizeCategory(itemRecord.category),
    };
  });
